package com.ambition.ecm.dcrn.service;
...
@Service
@Transactional
public class DcrnReportManager extends WorkflowManagerSupport<DcrnReport> implements FormFlowableDeleteInterface,RetrieveTaskInterface,
EndInstanceInterface{
	
	public static final String DCRN_CODE="ecm-dcrn";
	...
	public Page<DcrnReport> searchPage(Page<DcrnReport> page){
		return dcrnReportDao.searchPage(page);
	}
	/**
	  * 方法名: 获取对象当前的办理人员
	  * <p>功能说明：</p>
	  ...
	  * @return
	 */
	public List<String[]> getTaskHander(DcrnReport entity) {
	...
	public Boolean retrieveTask(DcrnReport entity){
		...
		List<Object> list  = dcrnReportDao.getSession().createSQLQuery(sql).list();
		if(list.size()>0){
	...
	public void submitProcess(DcrnReport report ,List<JSONObject> reportArrays) throws Exception{
		saveDcrnReport(report ,reportArrays);
		Long processId = ApiFactory.getDefinitionService().getWorkflowDefinitionsByCode(DCRN_CODE).get(0).getId();
	...
	/**
	  ...
	  * @return
	 */
	public void completeTaskCode(DcrnReport report ,List<JSONObject> reportArrays,Long taskId,TaskProcessingResult taskTransact) throws Exception{
		saveDcrnReport(report ,reportArrays);
...
//		}
	...
	public void saveDcrnReport(DcrnReport report ,List<JSONObject> reportArrays) throws Exception{
		if(report.getId()==null){
			report.setDcrnReportDetails(new ArrayList<DcrnReportDetail>());
		}
		if(reportArrays!=null){
			report.getDcrnReportDetails().clear();
			for(JSONObject obj:reportArrays){
				...
				dd.setDcrnReport(report);
				report.getDcrnReportDetails().add(dd);
			}
		}
		dcrnReportDao.save(report);
	}
	 ...
	 * @param entity 删除的对象
	 */
	public void deleteEntity(DcrnReport entity){
		if(entity.getWorkflowInfo()!=null){
			String workflowId =  entity.getWorkflowInfo().getWorkflowId();
			//先删除子表
			Long reportId = entity.getId();
			String sql31 = "delete from ECM_DCRN_REPORT_DETAIL where FK_DCRN_REPORT = ?";	
			
			dcrnReportDao.getSession().createSQLQuery(sql31)
			.setParameter(0,reportId)
			.executeUpdate();

			String sql = "delete from product_task_all_his where execution_id = ?";
			dcrnReportDao.getSession().createSQLQuery(sql)
			.setParameter(0,workflowId)
			.executeUpdate();
			dcrnReportDao.delete(entity);
	...
	public String deleteEntity(String ids) {
		...
		for (String id : deleteIds) {
			DcrnReport report = getEntity(Long.valueOf(id));
			deleteEntity(report);
			deleteNum++;
		}
	...
	public void deleteFormFlowable(Long arg0) {
		dcrnReportDao.delete(arg0);
	}
	@Override
	public DcrnReport getEntity(Long id) {
		return dcrnReportDao.get(id);
	}
	@Override
	protected void saveEntity(DcrnReport arg0) {
		// TODO Auto-generated method stub
     ...
     * @throws IOException
    */
   public void exportReport(DcrnReport s) throws IOException{
       InputStream inputStream = null;
       try {
    	   DcrnReport report = s;                                                
           inputStream = this.getClass().getClassLoader().getResourceAsStream("template/report/ecm_dcrn.xls");
           Map<String,ExportExcelFormatter> formatterMap = new HashMap<String, ExportExcelFormatter>();
           String exportFileName = "DCR/N变更单";
