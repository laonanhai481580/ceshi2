...
import com.ambition.aftersales.entity.LarTargetDefectiveItem;
...
public class LarTargetManager {
	...
	public LarTarget getByModel(String ofilmModel) {
		return larTargetDao.getByModel(ofilmModel);
	}
	//新增
	public String saveMailSettings(LarData larData,String warnManLogin){
		String message = "";
		...
		if(StringUtils.isNotEmpty(warnManLogin)){
			//发送邮件
			message = "*"+dateStr+"最新预警信息:"+larData.getBusinessUnitName()+"事业部"+sdf.format(larData.getLarDate())+","+larData.getOfilmModel()+"机种VLRR数据不良率超过了目标值!";
