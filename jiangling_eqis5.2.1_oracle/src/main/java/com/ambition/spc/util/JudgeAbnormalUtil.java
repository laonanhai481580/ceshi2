...
public class JudgeAbnormalUtil {
	...
	public static ArrayList BaseAbnormal1(double[] dataJu, Abnormity abnormal,
		...
		for (int j = 0; j < dataLength; j++) {
			...
			if ((j < dataLength - 1) && (data[j] < data[(j + 1)])) {
				if ((num < param1)
						&& ((dataJudgeState == null) || (!dataJudgeState[j]) || (!dataJudgeState[(j + 1)])))
					dataState = false;
				num++;
			} else {
				if ((num >= param1) && (!dataState)) {
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static ArrayList BaseAbnormal2(double[] dataJu, Abnormity abnormal,
			String[] dateTime, boolean[] dataJudgeState, String title,
			ArrayList retList, int upLow, int[] jump) {
		float[] data = doubleToFloat(dataJu);
		int num = 1;
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static ArrayList BaseAbnormal3(double[] dataJu, Abnormity abnormal,
			String[] dateTime, boolean[] dataJudgeState, String title,
			ArrayList retList, int upLow, int[] jump) {
		float[] data = doubleToFloat(dataJu);
		int num = 1;
	...
	public static ArrayList BaseAbnormal4(double[] dataJu, Abnormity abnormal,
		...
		for (int j = 0; j < dataLength - 1; j++) {
			...
			if ((j < dataLength - 2)
					&& (((data[j] > data[(j + 1)]) && (data[(j + 1)] < data[(j + 2)])) || ((data[j] < data[(j + 1)]) && (data[(j + 1)] > data[(j + 2)])))) {
				if ((num < param1)
						&& ((dataJudgeState == null) || (!dataJudgeState[j])
								|| (!dataJudgeState[(j + 1)]) || (!dataJudgeState[(j + 2)])))
					dataState = false;
				num++;
			} else {
				if ((num >= param1) && (!dataState)) {
		...
		for (int j = 0; j < data.length - param2 + 1; j++) {
			...
			for (int k = j; k <= j + param1 - 1; k++) {
				...
				if ((num < param2)
						&& ((dataJudgeState == null) || (!dataJudgeState[k])))
					dataState = false;
				if (((tu != null) && (data[k] > tu.doubleValue()))
						|| ((tl != null) && (data[k] < tl.doubleValue())))
					num++;
			}
			if (num >= param2) {
				if (!dataState) {
