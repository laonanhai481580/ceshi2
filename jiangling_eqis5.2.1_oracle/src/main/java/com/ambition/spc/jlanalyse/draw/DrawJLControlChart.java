 ...
 *
 */
public class DrawJLControlChart
{
  public static BufferedImage drawJLChart(JLControlChartParam jLControlChartData,Map<String,Object> showParamMap)
  {
  ...
  private static BufferedImage drawDynJLChart(int startside, String titleY, int samplingNum, int chartType, 
		  double[] arrayx, double[] arrayy, double ucl1, double lcl1, double cl1, double ucl2, double lcl2, 
		  double cl2, int controlSta, boolean[] abnormalPosUp, boolean[] abnormalPosDow, String[] dateTime, 
		  Double tu, Double tl, int imageHeight, int imageWidth, int topHeight, int bottomHeight, int leftWidth, 
		  int rightWidth, int dotWid, int showDot, int xCoor, int dataPrecision, String upTitle, String lowTitle,
		  Map<String,Object> showParamMap)
  {
    try
    {
      ...
      if ((tu != null) || (tl != null)) {
        ...
        if ((tl != null) && (lcl1 != cl1))
        {
          ...
          //显示规格线
          if("1".equals(showParamMap.get("showGuiGe"))){
        	  DrawDotLine.drawBreakLine(g, leftWidth, (int)ydh, 
        	            imageWidth - rightWidth, (int)ydh);
        	          g.drawString("LSL=" + StringUtil.formatdouble(tl_ls, dataPrecision), 
        ...
        if ((tu != null) && (ucl1 != cl1)) {
          ...
          if("1".equals(showParamMap.get("showGuiGe"))){
        	  DrawDotLine.drawBreakLine(g, leftWidth, (int)ydhu, 
        	          ...
        	          g.drawString("USL=" + StringUtil.formatdouble(tu_ls, dataPrecision), 
      ...
      g.drawString("UCL="+StringUtil.formatdouble(ucl1, dataPrecision), imageWidth - 80, 
        (int)(topHeight + coorH18_1 * 2.0D));
      g.drawString("CL="+StringUtil.formatdouble(cl1, dataPrecision), imageWidth - 65, 
        (int)(topHeight + coorH18_1 * 5.0D));
      g.drawString("LCL="+StringUtil.formatdouble(lcl1, dataPrecision), imageWidth - 65, 
        (int)(topHeight + coorH18_1 * 8.0D));
      ...
      g.drawString("UCL="+StringUtil.formatdouble(ucl2, dataPrecision), imageWidth - 65, 
        (int)(topHeight + coorH18_1 * 11.0D + mLowValue));
      g.drawString("CL="+StringUtil.formatdouble(cl2, dataPrecision), imageWidth - 65, 
        (int)(topHeight + coorH18_1 * 14.0D + mLowValue));

      g.drawString("LCL="+StringUtil.formatdouble(lcl2, dataPrecision), imageWidth - 65, 
      ...
      if (showDot > 1)
      {
        ...
        if (endDot > 0)
        {
          ...
          List<JLSampleData> dataList = (List<JLSampleData>)showParamMap.get("dataList");
          ...
          //绘制最大值和最小值的线
          //最大值
          caculateMaxAndMin(dataList,g,endDot,startside,leftInterval,dotWid,
        		  cl1,ucl1,topHeight,coorH18_1,yTu,yTuOld,lcl1,yTlOld,yTl,true);
          //最小值
          caculateMaxAndMin(dataList,g,endDot,startside,leftInterval,dotWid,
        		  cl1,ucl1,topHeight,coorH18_1,yTu,yTuOld,lcl1,yTlOld,yTl,false);
      ...
      if(points.size()>0){
    	  //存储当前页码和全部页数
    	  int currentPage = startside/showDot+1;
    	  int maxPages = arrayx.length/showDot;
    	  if(arrayx.length%showDot>0){
    		  maxPages++;
    	  }
    	  JSONObject firstJson = points.getJSONObject(0);
    	  firstJson.put("pageNo",currentPage);
    	  firstJson.put("maxPages",maxPages);
//    	  points.set(0,firstJson);
      }
   ...
   * @param yTl
   */
  private static void caculateMaxAndMin(List<JLSampleData> dataList,Graphics2D g,
		  int endDot,int startside,double leftInterval,double dotWid,double cl1,
		  double ucl1,double topHeight,double coorH18_1,double yTu,double yTuOld,double lcl1,
		  double yTlOld,double yTl,boolean isMax){
	  if(dataList == null){
		  return;
	  }
	  double mx1=0.0d;
	  double my1=0.0d;
	  for (int i = 0; i < endDot; i++) {
      	if(startside+i>dataList.size()-1){
      		break;
      	}
	      double mx = leftInterval + dotWid * i;
	      double my;
	      JLSampleData data = dataList.get((startside + i));
	      double value = data.getMax();
	      if(!isMax){
	    	  value = data.getMin();
	      }
          if (value > cl1)
...
//          color = Color.white;
          g.setColor(color);
          g.fillOval((int)mx - 3, (int)my - 3, 5, 5);
          if (i > 0)
          {
            g.setColor(color);
            g.drawLine((int)mx1, (int)my1, (int)mx, (int)my);
