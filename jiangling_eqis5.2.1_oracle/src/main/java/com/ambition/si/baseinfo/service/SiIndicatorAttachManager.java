...
import com.ambition.si.baseinfo.dao.SiIndicatorAttachDao;
 ...
 */
@Service
public class SiIndicatorAttachManager{
	private Logger log = Logger.getLogger(this.getClass());
	@Autowired
	private SiIndicatorAttachDao siIndicatorAttachDao;
	 ...
	 * @throws IOException 
	 */
	public SiIndicatorAttach addIndicatorAttach(File file,String fileName,String model,String modelName,Long companyId) throws IOException{
		SiIndicatorAttach attach = new SiIndicatorAttach();
		...
		try {
			...
			siIndicatorAttachDao.save(attach);
			Session session=siIndicatorAttachDao.getSession();
			session.flush();
	...
	public void downloadIISByAttachId(HttpServletResponse response,Long attachId) throws IOException{
		try {
			SiIndicatorAttach attach = siIndicatorAttachDao.getIndicatorAttachById(attachId);
			if(attach == null){
	 ...
	 * @throws IOException 
	 */
	public SiIndicatorAttach copyIndicatorAttach(Long indicatorId){
		if(indicatorId == null){
			return null;
		}
		SiIndicatorAttach sourceAttach = siIndicatorAttachDao.getIndicatorAttachById(indicatorId);
