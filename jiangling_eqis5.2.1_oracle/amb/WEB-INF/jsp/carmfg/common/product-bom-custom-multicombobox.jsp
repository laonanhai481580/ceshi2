		...
		<td colspan="3" style="padding:0px;background:red;" valign="middle">
			<div style="height:33px;line-height:33px;background:blue;">
				<div class="opt-btn" style="height:30px;line-height:30px;">
					物料名称/代号:<input></input>
					<button class="sexybutton" onclick="$('#<%=elementId%>').CustomMultiCombobox('search','<%=elementId%>_BomList',$(this).parent().find('input').val())"><span class="search">查询 </span></button>	
...
//	 				"initially_select" : [ "root"]
				...
				id = id == 'root'?'':id;
				$('#<%=elementId%>').CustomMultiCombobox('searchByParent','<%=elementId%>_BomList',id);
			});
		var colModel = [
	   		...
	   		{label:'名称',name:'name',index:'name', width:130,formatter:function(value,o,obj){
	   			if(obj.hasChild==true||obj.hasChild=='true'){
					return "<a class=\"small-button-bg\" onclick=\"window.loadStructure='yes';$('#<%=elementId%>').CustomMultiCombobox('searchByParent','<%=elementId%>_BomList',"+obj.id+");\" href=\"#\" style='float:left;margin-right:2px;'><span class=\"ui-icon ui-icon-folder-collapsed\"></span></a>" + value;
				}else if(value){
					return value;
				}else{
					return '';
...
// 	   			if(obj.hasChild==true||obj.hasChild=='true'){
<%-- 					return "<a class=\"small-button-bg\" onclick=\"window.loadStructure='yes';$('#<%=elementId%>').CustomMultiCombobox('searchByParent','<%=elementId%>_BomList',"+obj.id+");\" href=\"#\" style='float:left;margin-left:10px;' title='转到【"+obj.name+"】'><span class=\"ui-icon ui-icon-newwin\" style='cursor:pointer;'></span></a>"; --%>
// 				}else{
// 					return '';
// 				}
// 	   		}}		
		...
		$("#"+gridId).jqGrid({
			...
			loadComplete:function(){
				var value = $("#<%=elementId%>").CustomMultiCombobox("val");
			...
			url:'<%=request.getContextPath()%>/mfg/base-info/bom/list-datas.htm',
		   	...
		   	onSelectRow : function(rowid){
		   		if(window.loadStructure != 'yes'){
		   			$("#<%=elementId%>").CustomMultiCombobox("select","<%=elementId%>_BomList",rowid);
		   		}else{
		   			delete window.loadStructure;
		   		}
		   	},
		   	onSelectAll : function(selarrrow){
		   		$("#<%=elementId%>").CustomMultiCombobox("selectAll","<%=elementId%>_BomList",selarrrow);
		   	}
		});
		$("#<%=elementId%>").CustomMultiCombobox("layout","<%=elementId%>_BomList");
	});
</script>