...
//	 						rotation : -45,
						...
						formatter: function() {
							if(this.point.date){
								if(result.group == 'number'){
									...
									return '<b>' + this.x + ' 不良发生率:</b>' + this.y + "%<br/>单击编辑备注";
								}
							}else{
		...
		function reportByParams(params){
			$.post("${mfgctx}/other-manu-analyse/unquality-trend-chart-datas.htm",params,function(result){
				if(result.error){
		...
		function loadTableDataByDate(date){
			if(cacheTableResult&&date==hisDate){
				...
				var url = "${mfgctx}/other-manu-analyse/unquality-trend-table-datas.htm";
