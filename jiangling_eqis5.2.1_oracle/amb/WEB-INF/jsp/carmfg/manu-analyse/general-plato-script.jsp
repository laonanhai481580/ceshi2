...
//		 			        tickInterval : result.max/10,
						...
						plotOptions: {
							column: {
					            ...
					            events : {
					            	click : function(obj){
					            		showDetailByArg(obj.point.arg,obj.point.type);
					            	}
					            }
					        },
					        spline: {
							...
							formatter: function() {
								...
								if(this.point.name == 'spline'){
									...
									if("result"==result.type){
										s = ''+
										'不良扣分: '+ this.y + "<br/><span style='font-size:12px;color:blue;'>单击查看详细</span>";
									}else{
										s = ''+
		...
		function reportByParams(params){
			...
			$.post("${mfgctx}/manu-analyse/general-plato-datas.htm",params,function(result){
				$("#message").html("");
				if(result.error){
...
// 					window.location = "#reportDiv";
		...
		//查看明细的方法
		function showDetailByArg(arg,type){
			var params = getParams();
			params['params.arg'] = arg;
			var url = '${mfgctx}/manu-analyse/inspection-regular-right-detail.htm?_source=general';
			if(type == 'check'){
				url = '${mfgctx}/manu-analyse/check-regular-right-detail.htm?_source=general';
			}
