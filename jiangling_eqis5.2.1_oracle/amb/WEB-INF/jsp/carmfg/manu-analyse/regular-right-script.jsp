...
//	 			            groupPadding:0.0,
				            ...
				            events : {
				            	click : function(obj){
				            		showDetailByDate(obj.point.date,obj.point.type);
				            	}
				            }
				        },
				        line: {
...
// 			var val = $('input:checkbox[name="params.myType"]:checked').val();
		...
		function reportByParams(params){
			...
			$.post("${mfgctx}/manu-analyse/regular-right-chart-datas.htm",params,function(result){
				$("#message").html("");
				if(result.error){
		...
		function createDetailTable(result){
			clearTable();
			setTimeout(function(){
				var colModel=[{name:'custom_name',width:70,index:'custom_name',label:result.myType}],datas = [];
		...
		//查看明细的方法
		function showDetailByDate(inspectionDate,type){
			var params = getParams();
			delete params['params.startDate_ge_date'];
			delete params['params.endDate_le_date'];
			params['params.inspectionDate_ge_datetime'] = inspectionDate + " 00:00:00";
			params['params.inspectionDate_le_datetime'] = inspectionDate + " 23:59:59";
			var url = '${mfgctx}/manu-analyse/inspection-regular-right-detail.htm?a=1';
			if(type == 'check'){
				url = '${mfgctx}/manu-analyse/check-regular-right-detail.htm?a=1';
			}
		...
		function goToNewLocationById(id){
			var url='${mfgctx}';
			$.colorbox({href:url+"/manu-analyse/view-info.htm?id="+id,iframe:true, 
				innerWidth:$(window).width()<900?$(window).width()-50:900, 
				innerHeight:$(window).height()<700?$(window).height()-50:$(window).height(),
				overlayClose:false,
				title:"页面详情"
