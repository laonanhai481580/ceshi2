...
$(function(){
	...
	var attachments = [
		   				...
		   				{showInputId:'showattachment4',hiddenInputId:'attachment4'},
		   				{showInputId:'showattachment5',hiddenInputId:'attachment5'},
		   				{showInputId:'showattachment6',hiddenInputId:'attachment6'}
		   			];
...
function addForm(){
	window.location="${qsmctx}/inner-audit/correct-measures/input.htm";
}
//图片上传
...
function copyPersonCallBack(selectHiddenValue) {
	...
	$("#applicationForm").attr("action", "${qsmctx}/inner-audit/correct-measures/copytask.htm?copyPerson=" + selectHiddenValue);
	$("#applicationForm").ajaxSubmit(function(id) {
		alert(id);
	});
}
function assignUser(selectHiddenValue){
	var url ="${qsmctx}/inner-audit/correct-measures/assign.htm?assignee="+selectHiddenValue;
...
// 		}
		...
		$("#applicationForm").attr("action","${qsmctx}/inner-audit/correct-measures/save.htm");
	...
	function submitForm(){
		...
		$("#applicationForm").attr("action",'${qsmctx}/inner-audit/correct-measures/submit-process.htm');
		$("#applicationForm :input[name]").removeAttr("disabled");
		$("#applicationForm").submit();
	}
	...
	function _completeTask(taskTransact) {
		...
		$("#applicationForm :input[name]").removeAttr("disabled");
		$("#applicationForm").attr("action","${qsmctx}/inner-audit/correct-measures/complete-task.htm");
		var validator = $("#applicationForm").data('validator');
		if(validator){
			validator.cancelSubmit = true;
		}
