...
$(function(){
	...
	_validateForm();
	var attachments = [
		   				{showInputId:'showfileName',hiddenInputId:'fileName'},
		   				{showInputId:'showinspectionReportFile',hiddenInputId:'inspectionReportFile'},
		   				{showInputId:'showrdReportFile',hiddenInputId:'rdReportFile'},
		   				{showInputId:'showqsReportFile',hiddenInputId:'qsReportFile'},
		   				{showInputId:'showlabReportFile',hiddenInputId:'labReportFile'}
		   			];
...
function copyPersonCallBack(selectHiddenValue) {
	...
	$("#applicationForm").attr("action", "${supplierctx}/material-admit/evaluate/copytask.htm?copyPerson=" + selectHiddenValue);
	$("#applicationForm").ajaxSubmit(function(id) {
		alert(id);
	});
}
function assignUser(selectHiddenValue){
	var url ="${supplierctx}/material-admit/evaluate/assign.htm?assignee="+selectHiddenValue;
...
// 		}
		...
		$("#applicationForm").attr("action","${supplierctx}/material-admit/evaluate/save.htm");
	...
	function submitForm(){
		...
		$("#applicationForm").attr("action",'${supplierctx}/material-admit/evaluate/submit-process.htm');
		$("#applicationForm :input[name]").removeAttr("disabled");
		$("#applicationForm").submit();
	}
	...
	function _completeTask(taskTransact) {
		...
		$("#applicationForm :input[name]").removeAttr("disabled");
		$("#applicationForm").attr("action","${supplierctx}/material-admit/evaluate/complete-task.htm");
		var validator = $("#applicationForm").data('validator');
		if(validator){
			validator.cancelSubmit = true;
		}
...
// 		}   
  //file
      ...
      function showManager(obj){
    	  var toManager = obj.value;
    	  var sbtitle=document.getElementById("managerId");
    	  if(toManager=="是"){
  			if(sbtitle){
  				 sbtitle.style.display='block';
  	        }
