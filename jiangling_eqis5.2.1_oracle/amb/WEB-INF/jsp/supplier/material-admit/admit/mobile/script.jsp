...
var selectValue="",selectHiddenValue="";
    $(function(){
    	...
    	var attachments = [
		   		{showInputId:'showengineeringDrawing',hiddenInputId:'engineeringDrawing'},
		   		{showInputId:'showcheckoutReport',hiddenInputId:'checkoutReport'},
		   		{showInputId:'showpacking',hiddenInputId:'packing'},
		   		{showInputId:'showcodeExplain',hiddenInputId:'codeExplain'},
		   		{showInputId:'showenvironmentalMaterial',hiddenInputId:'environmentalMaterial'}
		   	];
		...
		var admitProjects = '${admitProject}'.split(",");
		$(":input[name=admit_select]").each(function(index,obj){
			var checked = false;
			for(var i=0;i<admitProjects.length;i++){
				if($.trim(obj.value)==$.trim(admitProjects[i])){
					checked = true;
					break;
				}
			}
   ...
   function setOpinionValue(obj){
	...
	function getAdmitProjects(){
 	...
 	function saveForm() {
 		...
 		$("#applicationForm").attr("action","${supplierctx}/material-admit/admit/save.htm");
 	...
 	function submitForm(){
 		...
 		$("#applicationForm").attr("action",'${supplierctx}/material-admit/admit/submit-process.htm');
 		$("#applicationForm :input[name]").removeAttr("disabled");
 		$("#applicationForm").submit();
 	}
 	...
 	function _completeTask(taskTransact) {
 		...
 		$("#applicationForm :input[name]").removeAttr("disabled");
 		$("#applicationForm").attr("action","${supplierctx}/material-admit/admit/complete-task.htm");
 		var validator = $("#applicationForm").data('validator');
 		if(validator){
 			validator.cancelSubmit = true;
 		}
	...
	//添加验证
 	...
 	function copyPersonCallBack(selectHiddenValue) {
 		...
 		$("#applicationForm").attr("action", "${supplierctx}/material-admit/admit/copytask.htm?copyPerson=" + selectHiddenValue);
 		$("#applicationForm").ajaxSubmit(function(id) {
 			alert(id);
 		});
 	}
 	function assignUser(selectHiddenValue){
 		var url ="${supplierctx}/material-admit/admit/assign.htm?assignee="+selectHiddenValue;
