...
$(function(){
	...
	var attachments = [
		   				...
		   				{showInputId:'showthreePapersFile',hiddenInputId:'threePapersFile'},
		   				{showInputId:'showintegrityAgreementFile',hiddenInputId:'integrityAgreementFile'},
		   				{showInputId:'showsampleEvaluateFile',hiddenInputId:'sampleEvaluateFile'},
		   				{showInputId:'showfactoryAuditFile',hiddenInputId:'factoryAuditFile'},
		   				{showInputId:'showpurchasingFile',hiddenInputId:'purchasingFile'},
		   				{showInputId:'showsupplierAnalyzeFile',hiddenInputId:'supplierAnalyzeFile'}
		   			];
...
function minsize(){
	 var $formNoDiv=$(".formNoDiv");
	 var $formNoText=$(".formNoText");
	 if($formNoText.height() > $formNoDiv.height()){
	        $formNoText.css('font-size',parseInt($formNoText.css('font-size'))-5 +'px');
...
function parseDownloadFiles(arrs){
	for(var i=0;i<arrs.length;i++){
		...
		var filesLength = $("#"+hiddenInputId).val().length;
		if(filesLength>0){
	 		
	 		var files =  $("#"+hiddenInputId).val();
	 		var filesArr = files.split("s~s");
	 		var showHtml = "";
	 		for(var k=0;k<filesArr.length;k++){
	 			var fileName =  filesArr[k].split("|~|")[1];
	 			var fileId =   filesArr[k].split("|~|")[0];
	 			if(showHtml.length ==0){
	 				showHtml = '<a style="text-decoration:underline;" href="${mfgctx}/common/download.htm?id='+fileId+'" title="下载'+fileName+'">'+fileName+'</a>'
	 			}else{
	 				showHtml += ","+'<a style="text-decoration:underline;" href="${mfgctx}/common/download.htm?id='+fileId+'" title="下载'+fileName+'">'+fileName+'</a>';
	 			}
	 		}
	 		$("#"+showInputId).html(showHtml);
		}
	}
}
//图片上传
...
function copyPersonCallBack(selectHiddenValue) {
	...
	$("#applicationForm").attr("action", "${supplierctx}/develop/copytask.htm?copyPerson=" + selectHiddenValue);
	$("#applicationForm").ajaxSubmit(function(id) {
		alert(id);
	});
}
function assignUser(selectHiddenValue){
	var url ="${supplierctx}/develop/assign.htm?assignee="+selectHiddenValue;
...
// 		}
		...
		$("#applicationForm").attr("action","${supplierctx}/develop/save.htm");
	...
	function submitForm(){
		...
		$("#applicationForm").attr("action",'${supplierctx}/develop/submit-process.htm');
		$("#applicationForm :input[name]").removeAttr("disabled");
		$("#applicationForm").submit();
	}
	...
	function _completeTask(taskTransact) {
		...
		$("#applicationForm :input[name]").removeAttr("disabled");
		$("#applicationForm").attr("action","${supplierctx}/develop/complete-task.htm");
		var validator = $("#applicationForm").data('validator');
		if(validator){
			validator.cancelSubmit = true;
		}
