...
var selectValue="",selectHiddenValue="";
    $(function(){
    	...
    	_validateForm();
    	var attachments = [
	   		{showInputId:'showattachment1',hiddenInputId:'attachment1'},
	   		{showInputId:'showattachment2',hiddenInputId:'attachment2'},
	   		{showInputId:'showattachment3',hiddenInputId:'attachment3'}
	   	 ];
	...
	function $img(inputId,picBoxId){
 	...
 	function saveForm() {
 		...
 		$("#applicationForm").attr("action","${aftersalesctx}/far/save.htm");
 	...
 	function submitForm(){
 		...
 		$("#applicationForm").attr("action",'${aftersalesctx}/far/submit-process.htm');
 		$("#applicationForm :input[name]").removeAttr("disabled");
 		$("#applicationForm").submit();
 	}
 	//存子表
	function getCheckItemStrs(){
		var checkItemStrs = "";
		$(".operation li[class=operation_lithr]").each(function(index,obj){
			if(checkItemStrs){
				checkItemStrs += ",";
			}
			var str = '';
 	...
 	function _completeTask(taskTransact) {
 		...
 		$("#applicationForm :input[name]").removeAttr("disabled");
 		$("#applicationForm").attr("action","${aftersalesctx}/far/complete-task.htm");
 		var validator = $("#applicationForm").data('validator');
 		if(validator){
 			validator.cancelSubmit = true;
 		}
	...
	//添加验证
 	...
 	function copyPersonCallBack(selectHiddenValue) {
 		...
 		$("#applicationForm").attr("action", "${aftersalesctx}/far/copytask.htm?copyPerson=" + selectHiddenValue);
 		$("#applicationForm").ajaxSubmit(function(id) {
 			alert(id);
 		});
 	}
 	function assignUser(selectHiddenValue){
 		var url ="${aftersalesctx}/far/assign.htm?assignee="+selectHiddenValue;
	...
	function setOpinionValue(obj){
 	...
 	function setInputValue(){
		...
		function setFileinfo(obj){
			$("#"+showFileId).html('<a style="text-decoration:underline;margin-left:-4rem;" href="${mfgctx}/common/download.htm?id='+obj.fileId+'" title="下载'+obj.fileName+'">'+obj.fileName+'</a>');
		...
		function parseDownloadFiles(arrs){
			for(var i=0;i<arrs.length;i++){
				...
				if(files>0){
			 		...
			 		$("#"+showInputId).html('<a style="text-decoration:underline;margin-left:-4rem;" href="${mfgctx}/common/download.htm?id='+fileId+'" title="下载'+fileName+'">'+fileName+'</a>');
				}
			}
		}
