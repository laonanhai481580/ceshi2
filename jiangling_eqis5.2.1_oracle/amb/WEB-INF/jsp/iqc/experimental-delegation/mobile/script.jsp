...
var selectValue="",selectHiddenValue="";
	...
	function $img(inputId,picBoxId){
 	...
 	function saveForm() {
 		...
 		$("#applicationForm").attr("action","${iqcctx}/experimental-delegation/save.htm");
	...
	function getCheckItemStrs(){
 	...
 	function submitForm(){
 		...
 		$("#applicationForm").attr("action",'${iqcctx}/experimental-delegation/submit-process.htm');
 		$("#applicationForm :input[name]").removeAttr("disabled");
 		$("#applicationForm").submit();
 	}
 	...
 	function _completeTask(taskTransact) {
 		...
 		$("#applicationForm :input[name]").removeAttr("disabled");
 		$("#applicationForm").attr("action","${iqcctx}/experimental-delegation/complete-task.htm");
 		var validator = $("#applicationForm").data('validator');
 		if(validator){
 			validator.cancelSubmit = true;
 		}
	...
	//添加验证
 	...
 	function copyPersonCallBack(selectHiddenValue) {
 		...
 		$("#applicationForm").attr("action", "${iqcctx}/experimental-delegation/copytask.htm?copyPerson=" + selectHiddenValue);
 		$("#applicationForm").ajaxSubmit(function(id) {
 			alert(id);
 		});
 	}
 	function assignUser(selectHiddenValue){
 		var url ="${iqcctx}/experimental-delegation/assign.htm?assignee="+selectHiddenValue;
