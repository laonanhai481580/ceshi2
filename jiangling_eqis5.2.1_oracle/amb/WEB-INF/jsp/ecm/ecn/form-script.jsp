...
<%@page import="com.ambition.ecm.entity.EcnReportDetail"%>
...
/* 		//根据流程节点编号，制定会签环节填写内容权限
		if('${workCode}'=='stage3'){
			$("tr[isEdit]").each(function(index,obj){
				if('${loginDev}'.indexOf($(obj).attr("isEdit"))==-1){
					$(obj).find(":input").attr("disabled","disabled");
				}
			});
		} */
	...
	function submitForm(url){
		if($("#inputForm").valid()){
			...
			$("tr[tableName=ecnReportDetails]").find(":input").removeAttr("disabled");
	...
	//保存form
	function saveTask() {
// 		$("#inputForm").attr("action","${ecmctx}/ecn/save.htm?processTaskSave=true");
		 submitForm("${ecmctx}/ecn/save.htm?processTaskSave=true");
// 		$('#inputForm').submit();
	}
	//办理完任务关闭窗口前执行
	function beforeCloseWindow(opt) {
		aa.submit("defaultForm1", "${ecmctx}/ecn/process-task.htm", 'btnZone,viewZone');
	}
...
//选择减签人员
	...
	workflowButtonGroup.btnGetBackTask.click = function(taskId) {
		$.post("${ecmctx}/ecn/retrievetask.htm?taskId=${taskId}",{},function(str){
			alert(str);
	...
	function changeViewSet(opt) {
		if (opt == "history") {
			$("#tabs-2").load("${ecmctx}/ecn/show-history.htm?taskId=${taskId}",function(){
	...
	workflowButtonGroup.btnDrawTask.click = function(taskId) {
		aa.submit("inputForm",
				"${ecmctx}/ecn/receive-task.htm", 'main',
				receiveback);
	};
	...
	workflowButtonGroup.btnAssign.click = function(taskId) {
		$.colorbox({
			href : "${ecmctx}/ecn/assign-tree.htm"
	...
	function viewFormInfo(){
		...
		$.colorbox({href:'${ecmctx}/ecn/view-info.htm?id='+formId+"#tabs-2",iframe:true,
