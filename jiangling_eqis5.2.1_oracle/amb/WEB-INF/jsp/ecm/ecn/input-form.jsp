...
<h3 style="text-align: center;font-size: 25px;font-weight: bold;">工程变更通知单(ECN)</h3>
...
<table  class="form-table-border-left" style="width:100%;margin: auto;border:0px;">
	<caption style="text-align: right;font-weight: bold;">ECN&nbsp;NO: ${ecnNo}</caption>
	<tr  lanuch="stage1">
		<td>分发部门</td>
	...
	<tr lanuch="stage1">
		...
		<td colspan="2">
			<s:iterator value="ecm_dcrn_type" var="option">
				<input label="true" type="checkbox" id="${option.id}" name="dcrnType" value="${option.name}" <s:if test="%{#option.value.equals(dcrnType)}">checked="checked"</s:if> onchange="checkboxChange(this);"/>
				<label for="${option.id}" id="${option.id}_label">${option.name}</label>
			</s:iterator>
		</td>
		<td>签核时效</td>
		<td colspan="2">
			<s:iterator value="ecm_dcrn_peroid" var="option">
				<input type="radio" id="${option.id}" name="nuclearSignPeroid" value="${option.name}" <s:if test="%{#option.value.equals(nuclearSignPeroid)}">checked="checked"</s:if>/>
	...
	<tr lanuch="stage1">
		<td colspan="5">
			<s:iterator value="ecm_dcrn_cause" var="option">
				<input label="true" type="checkbox" id="${option.id}" name="causeType" value="${option.name}" <s:if test="%{#option.value.equals(causeType)}">checked="checked"</s:if> onchange="checkboxChange(this);"/>
				<label for="${option.id}" id="${option.id}_label">${option.name}</label>
			</s:iterator>
		</td>
	</tr>
	<tr lanuch="stage1">
		...
		<td colspan="5">
			<s:iterator value="ecm_ecm_context" var="option">
				<input label="true" type="checkbox" id="${option.id}" name="alterationContent" value="${option.name}" <s:if test="%{#option.value.equals(alterationContent)}">checked="checked"</s:if> onchange="checkboxChange(this);"/>
				<label for="${option.id}" id="${option.id}_label">${option.name}</label>
			</s:iterator>
		</td>
	</tr>
	<tr lanuch="stage1">
		...
		<td colspan="5">
			<table   class="form-table-border-left" style="width:100%;margin: auto;border:0px;">
				...
				<s:iterator value="ecnReportDetails">
				<%
					flagStr.append(",flag" + a);
				%>
				<tr tableName="ecnReportDetails" name="ecnReportDetails" class="ecnReportDetails">
					<td style="width: 10%;">
...
<%-- 			<input type="hidden" name="raFileAttach" id="raFileAttach" isFile="true" uploadBtnText="上传附件"  value="${raFileAttach}"/> --%>
			<input type="hidden" id="raFileAttach" name="raFileAttach" value='${raFileAttach}'></input>
			<button  class='btn' type="button" onclick='uploadFiles("showRaFileAttach","raFileAttach");'><span><span><b class="btn-icons btn-icons-upload"></b>上传附件</span></span></button>
			<span id="showRaFileAttach"></span>
		</td>
		<td>测试报告编号</td>
		<td>
...
<%-- 			<input type="hidden" name="tryFileAttach" id="tryFileAttach" isFile="true" uploadBtnText="上传附件"  value="${tryFileAttach}"/> --%>
	...
	<tr lanuch="stage1">
		...
		<td>附件</td>
		<td>
<%-- 			<input type="hidden" id="harmfulFileAttach"  isFile="true" name="harmfulFileAttach" value="${harmfulFileAttach}"/> --%>
	...
	<tr  lanuch="stage1">
		...
		<td>
			...
			<%-- <s:date name="personLiableTime" format="yyyy-MM-dd HH:mm"/> --%>
		</td>
		<td></td>
		<td>
		</td>
		...
		<td colspan="5">
			<table style="width:100%;border: 0px;">
				...
				<tr>
					...
					<td style="border: 0px;">
						<label style="width: 80px;display: block;float: left;">事业部:</label>
						<input type="hidden" id="businessUnitLogin" login="true" name="businessUnitLogin" value="${businessUnitLogin}"/>
						<input id="businessUnit" name="businessUnit" isLogin="true" value="${businessUnit}" readonly="readonly"/>
					</td>
					<td style="border: 0px;">
						<label style="width: 80px;display: block;float: left;">事业群:</label>
						<input type="hidden" id="businessGroupLogin" login="true" name="businessGroupLogin" value="${businessGroupLogin}"/>
						<input id="businessGroup" name="businessGroup" isLogin="true" value="${businessGroup}" readonly="readonly"/>
					</td>
					<td style="border: 0px;">
					</td>
				</tr>				
	...
	<tr lanuch="stage3" isEdit="开发" edit="PM">
		<td>PM </td>
		<td colspan="2">
			<textarea rows="" cols="" name="pmSignText">${pmSignText}</textarea>
		</td>
		<td colspan="3">
			<table   class="form-table-border-left" style="width:100%;margin: auto;border:0px;">
				<tr>
				...
				<tr tableName="ecnReportDetails" class="ecnReportDetails">
					<td>${beforeCode}</td>
					<td>${beforeName}</td>
					<td>
	...
	<tr lanuch="stage3" isEdit="EE"  edit="EE">
		<td>EE</td>
		<td colspan="5">
			<textarea rows="2" cols="" name="eeSignText">${eeSignText}</textarea>
		</td>
	</tr>
	<tr lanuch="stage3" isEdit="ME" edit="ME">
		<td>ME</td>
		<td colspan="5">
			<textarea rows="" cols=""  name="meSignText">${meSignText}</textarea>		
		</td>
	</tr>
	<tr lanuch="stage3" isEdit="材料" edit="material">
		<td>材料组</td>
		<td colspan="5">
			<textarea rows="" cols="" name="materialSignText">${materialSignText}</textarea>					
		</td>
	</tr>
	<tr lanuch="stage3" isEdit="工程" edit="project">
		<td>工程组</td>
		<td colspan="2">
	...
	<tr lanuch="stage3" isEdit="PMC" edit="PMC">
		<td>PMC部</td>
		<td colspan="5">
			<table   class="form-table-border-left" style="width:100%;margin: auto;border:0px;">
				<tr>
...
<%-- 				<input type="hidden" isFile="true" uploadBtnText="上传附件"  id="qsSignFile" name="qsSignFile" value="${qsSignFile}"/> --%>
	...
	<tr lanuch="stage3" isEdit="事业部" edit="businessUnit">
		<td>事业部负责人</td>
		<td colspan="5">
			<textarea rows="3" cols="3" id="businessUnitText" name="businessUnitText">${businessUnitText}</textarea>
		</td>
	</tr>
	<tr lanuch="stage3" isEdit="事业群" edit="businessGroup">
		<td>事业群负责人</td>
		<td colspan="5">
			<textarea rows="3" cols="3" id="businessGroupText" name="businessGroupText">${businessGroupText}</textarea>
		</td>
	</tr>
